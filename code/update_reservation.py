...
def update_reservation():
    ...
    #---------------------------changing interface-------------------------------------------
    def add_reservation_menu(event):
        ...
        def choose_house():
            def add_reservation():
                ...
                nemra_simana = ww.get()
                # Create Table
                if  nemra_dar and nemra_simana:
                    answer = askyesno(title='confirmation', message='Are you sure that you want to update?')
                    if answer:
                        data_insert_query = '''Update reservation set num_maison = ?, num_semaine = ? where num_reservation = ?'''
                        data_insert_tuple = (nemra_dar,nemra_simana, id)
            ...
            num_maison_entry.set(str(result[0][2]))
            ...
            num_semaine_combobox.set(str(result[0][3]))
            ...
            #====
            button = tk.Button(frame, text="Update reservation", command=add_reservation)
        ...
        def choose_week():
            ...
            num_week_entry.set(str(result[0][3]))
            ...
            num_house_combobox.set(str(result[0][2]))
        ...
        query = 'select * from reservation where num_reservation={n}'.format(n=id)
        cursor.execute(query)
        result=cursor.fetchall()

        id_client_label= tk.Label(user_info_frame,text="id client : {n}".format(n=result[0][1]))
    ...
    cursor.execute(query)
    all_reservation_id = cursor.fetchall()
    for i in range(len(all_reservation_id)):
        all_reservation_id[i]=all_reservation_id[i][0]

    #id frame
    id_reservation_frame = tk.LabelFrame(frame, text="reservation id")
    id_reservation_frame.grid(row= 0, column=0, padx=20, pady=10)
    #select an id
    
    id_label = tk.Label(id_reservation_frame, text="Enter the reservation id : ", padx=20, pady=10)
    id_label.grid(row=0, column=0)
    id_combobox =  ttk.Combobox(id_reservation_frame, values=all_reservation_id, state="readonly")
    id_combobox.grid(row=0, column=1)

    test=tk.Label(id_reservation_frame, text="", padx=40, pady=10)
    test.grid(row=0, column=2)

    id_combobox.current()
    id_combobox.bind("<<ComboboxSelected>>", add_reservation_menu)
